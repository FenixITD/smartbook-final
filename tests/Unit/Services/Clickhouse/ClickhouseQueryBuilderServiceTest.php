...
class ClickhouseQueryBuilderServiceTest extends TestCase
{
    ...
    public function test_where_adds_string_condition(): void
    {
        $this->manager->expects('select')->with(
            "SELECT * FROM test_table WHERE col = {col_0:String} ORDER BY created_at DESC",
            ['col_0' => 'val'],
        )->andReturn([]);
    ...
    {
        $this->manager->expects('select')->with(
            "SELECT * FROM test_table WHERE col = {col_0:UInt64} ORDER BY created_at DESC",
            ['col_0' => 123],
        )->andReturn([]);
    ...
    {
        $this->manager->expects('select')->with(
            "SELECT * FROM test_table WHERE col IN ({col_0:String},{col_1:String}) ORDER BY created_at DESC",
            ['col_0' => 'a', 'col_1' => 'b'],
        )->andReturn([]);
    ...
    public function test_final_appends_final_to_count(): void
    {
        $this->manager->expects('count')->with("SELECT count() as count FROM test_table FINAL", [])->andReturn(3);

        $this->assertSame(3, $this->builder->final()->count());
    }

    public function test_final_appends_final_to_get(): void
    {
        $this->manager->expects('select')->with("SELECT * FROM test_table FINAL ORDER BY created_at DESC", [])->andReturn([['id' => 1]]);

        $this->builder->final()->get();
    }

    public function test_final_with_where(): void
    {
        $this->manager->expects('select')->with(
            "SELECT * FROM test_table FINAL WHERE status = {status_0:String} ORDER BY created_at DESC",
            ['status_0' => 'active'],
        )->andReturn([]);

        $this->builder->final()->where('status', 'active')->get();
    }
}
