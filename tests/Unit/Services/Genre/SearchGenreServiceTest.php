...
class SearchGenreServiceTest extends TestCase
{
    ...
    private SearchGenreService $service;

    protected function setUp(): void
    {
        parent::setUp();
        $this->client = Mockery::mock(Client::class);
        $this->service = new SearchGenreService($this->client);
    }
    ...
    public function test_returns_ids_and_total(): void
    {
        ...
        [$ids, $total] = $this->service->search(new GenreFiltersDto());

        $this->assertSame([1, 2], $ids);
        $this->assertSame(2, $total);
    }

    public function test_uses_match_all_when_search_is_empty(): void
    {
        ...
        $this->service->search(new GenreFiltersDto());
    }
    ...
    public function test_uses_multi_match_when_search_is_provided(): void
    {
        $this->client->expects('search')->withArgs(function (array $params): bool {
            ...
            return isset($query['bool']['must'][0]['multi_match']['query'])
                && $query['bool']['must'][0]['multi_match']['query'] === 'Fantasy';
        })->andReturn($this->makeElasticsearchResponse([]));

        $this->service->search(new GenreFiltersDto(search: 'Fantasy'));
    }
}
