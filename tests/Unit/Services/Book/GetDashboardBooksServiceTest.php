...
class GetDashboardBooksServiceTest extends TestCase
{
    ...
    public function test_returns_empty_paginated_when_no_ids_found(): void
    {
        ...
        $this->searchService->expects('search')->with($filters)->andReturn([[], 0]);
    ...
    public function test_returns_repo_results_when_ids_found(): void
    {
        ...
        $this->searchService->expects('search')->with($filters)->andReturn([[1, 2], 2]);
        $this->repository->expects('getDashboardListByIds')->with([1, 2], 2, $filters)->andReturn($paginated);
