    ...
    public function saving(ClickhouseActivity $activity): bool
    {
        ...
        DB::afterCommit(function () use ($row): void {
            try {
                ...
                $maxLen = config('clickhouse.stream_max_len', 100_000);
                $maxLenInt = is_numeric($maxLen) ? (int) $maxLen : 100_000;
                Redis::xadd('clickhouse_activities_stream', '*', ['payload' => $payload], $maxLenInt, false);
            } catch (Throwable $e) {
