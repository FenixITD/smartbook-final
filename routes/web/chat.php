...
Route::middleware(['auth'])->prefix('chat')->name('chat.')->group(function (): void {
    ...
    Route::get('/conversation/{conversation}/messages', GetMessageController::class)->name('messages.index')->whereNumber('conversation');
    Route::post('/conversation/{conversation}/messages', SendMessageController::class)->name('messages.store')->whereNumber('conversation');
    Route::middleware(['admin'])->group(function (): void {
        Route::get('/admin', AdminConversationController::class)->name('admin');
        Route::patch('/conversation/{conversation}/close', CloseConversationController::class)->name('conversation.close')->whereNumber('conversation');
    });
});
