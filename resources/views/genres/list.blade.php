<x-layouts::app.header title="Genres">
    <div class="flex min-h-screen flex-col">
        ...
        <div class="flex-1 flex flex-col gap-6 p-6">
            ...
            <div class="flex items-center justify-between">
                <div>
                    <flux:heading size="xl">Genres</flux:heading>
                    <flux:text class="mt-1 text-zinc-500">Manage book genres</flux:text>
                </div>
                <flux:button href="{{ route('genres.create') }}" variant="primary" icon="plus">
                    Add genre
                </flux:button>
            </div>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                <form method="GET" action="{{ route('genres.index') }}" class="flex flex-wrap gap-3 items-end">
                    <div class="flex-1 min-w-48 relative" x-data="genreSuggest()" x-ref="wrapper">
                        <flux:input
                            name="search"
                            x-model="query"
                                        ...
                                        <li>
                                            <a :href="item.url"
                                               class="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer">
                                                <div class="flex h-8 w-8 items-center justify-center rounded-full bg-purple-100 text-purple-700 text-xs font-bold dark:bg-purple-900 dark:text-purple-300"                                                     x-text="item.name.charAt(0).toUpperCase()">
                                                </div>
                                                <span class="text-sm font-medium text-zinc-900 dark:text-zinc-100" x-text="item.name"></span>
                                            </a>
                                        </li>
                                    </template>
                                </ul>
                    ...
                    @if(request('search') || request('sortBy'))
                        <flux:button href="{{ route('genres.index') }}" variant="ghost">Reset</flux:button>
                    @endif
                </form>
            </div>
            ...
            <div class="rounded-xl border border-zinc-200 bg-white overflow-hidden dark:border-zinc-700 dark:bg-zinc-900 flex-1 flex flex-col">
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-zinc-200 dark:divide-zinc-700">
                        ...
                        @forelse ($paginated->items as $genre)
                            <tr class="hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors">
                                ...
                                <td class="px-6 py-4">
                                    <div class="flex items-center gap-3">
                                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-purple-100 text-purple-700 text-xs font-bold dark:bg-purple-900 dark:text-purple-300">
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <flux:button href="{{ route('genres.show', $genre->id) }}" variant="ghost" size="sm" icon="eye">View</flux:button>
                                        <flux:button href="{{ route('genres.edit', $genre->id) }}" variant="ghost" size="sm" icon="pencil">Edit</flux:button>
                                        <div>
                                            <flux:modal.trigger name="delete-genre-{{ $genre->id }}">
                                                <flux:button variant="ghost" size="sm" icon="trash">Delete</flux:button>
                                            </flux:modal.trigger>

                                            <flux:modal name="delete-genre-{{ $genre->id }}" class="min-w-[22rem] text-left">
                                                <flux:heading size="lg">Delete genre?</flux:heading>
                                                <flux:subheading>Are you sure you want to delete "{{ $genre->name }}"?</flux:subheading>
                            ...
                            <tr>
                                <td colspan="4" class="px-6 py-16 text-center">
                                    <div class="flex flex-col items-center gap-3 text-zinc-400">
                                        <flux:icon name="tag" class="w-10 h-10" />
                                        <span class="text-sm">No genres yet</span>
                                        <flux:button href="{{ route('genres.create') }}" variant="primary" size="sm">
                                            Add first genre
                                        </flux:button>
                                    </div>
                                </td>
        ...
        function genreSuggest() {
            return {
                ...
                async fetchSuggestions() {
                    ...
                    try {
                        const res = await fetch('{{ route('api.genres.suggest') }}?q=' + encodeURIComponent(q), {
