...
<x-layouts::app.header title="Recent Activities">
    <div class="flex min-h-screen flex-col">
        <div class="flex-1 flex flex-col gap-6 p-6 max-w-3xl mx-auto w-full">
            ...
            @if ($logs->items === [])
                <div
                    class="rounded-xl border border-zinc-200 bg-white dark:border-zinc-700 dark:bg-zinc-900 px-6 py-16 text-center">
                    ...
                    <div class="mt-4">
                        <flux:button href="{{ route('dashboard') }}" variant="primary" size="sm">Browse books
                        </flux:button>
                    </div>
                </div>
            @else
                <div class="flex flex-col gap-3">
                    @foreach ($logs->items as $log)
                        @php
                            ...
                            $commentText = $isReview ? ($log->properties['attributes']['comment'] ?? null) : null;
                            $rating = $isReview ? ($log->properties['attributes']['rating'] ?? null) : null;
                        ...
                        <div
                            class="flex items-start gap-4 rounded-xl border {{ $borderColor }} {{ $bgColor }} px-5 py-4">

                            <div
                                class="flex-shrink-0 flex items-center justify-center w-10 h-10 rounded-full bg-white dark:bg-zinc-900 shadow-sm mt-0.5">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                @if ($isReview && $commentText)
                                    <div
                                        class="mt-2 text-sm text-zinc-600 dark:text-zinc-400 italic bg-white/50 dark:bg-black/20 p-2 rounded-lg border border-purple-100/50 dark:border-purple-900/50">
                                        @if($rating)
                                            <span class="font-semibold text-purple-600 dark:text-purple-400 mr-1">★ {{ $rating }}/5</span>
                                        @endif
                                        "{{ Str::limit($commentText, 150) }}"
                                    </div>
                                @endif
                            </div>

                            <p class="flex-shrink-0 text-xs text-zinc-400 whitespace-nowrap pt-1">
                                {{ $log->createdAt }}
                            </p>
