        ...
        <div class="flex-1 flex flex-col gap-6 p-6">
            ...
            <div class="rounded-xl border border-zinc-200 bg-white p-4 dark:border-zinc-700 dark:bg-zinc-900">
                <form method="GET" action="{{ route('reviews.index') }}" class="flex flex-wrap gap-3 items-end">
                    <div class="flex-1 min-w-48 relative" x-data="reviewSuggest()" x-ref="wrapper">
                        <flux:input
                            name="search"
                            x-model="query"
                                        ...
                                        <li>
                                            <a :href="item.url"
                                               class="flex items-center gap-3 px-4 py-3 hover:bg-zinc-50 dark:hover:bg-zinc-800 transition-colors cursor-pointer">
                                                ...
                                                <span class="text-sm text-zinc-500 dark:text-zinc-400 truncate" x-text="item.content"></span>
                                            </a>
                                        </li>
                                    </template>
                                </ul>
                                ...
                                <td class="px-6 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        ...
                                        <div>
                                            ...
                                            <flux:modal name="delete-review-{{ $review->id }}" class="min-w-[22rem] text-left">
                                                <flux:heading size="lg">Delete review?</flux:heading>
                                                <flux:subheading>Are you sure you want to delete review #{{ $review->id }}?</flux:subheading>
        ...
        function reviewSuggest() {
            return {
                ...
                async fetchSuggestions() {
                    ...
                    try {
                        const res = await fetch('{{ route('api.reviews.suggest') }}?q=' + encodeURIComponent(q), {
